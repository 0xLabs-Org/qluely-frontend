                    ...
                    <div className="lg:ml-auto flex-shrink-0">
                        <AlertDialog open={open} onOpenChange={setOpen}>
                            ...
                            <AlertDialogContent>
                                ...
                                <div className="mt-4">
                                    <div className="mb-3 font-medium text-sm text-[var(--dash-text-secondary)]">Plan</div>
                                    <div className="grid grid-cols-1 gap-2">
                                        {availablePlansForCurrent(plan).map((p) => (
                                            <label
                                                key={p}
                                                className={`p-3 border rounded-lg cursor-pointer flex items-center transition-all ${selectedPlan === p ? 'border-[var(--dash-accent)] bg-blue-50/50' : 'border-[var(--dash-border)] hover:border-[var(--dash-text-muted)]'
                                                    }`}
                                            >
                                                <input
                                                    type="radio"
                                                ...
                                                />
                                                <span className="text-sm font-medium text-[var(--dash-text-primary)]">{planDisplayNames[p]} Plan</span>
                                            </label>
                                        ))}
                                    </div>

                                    <div className="mt-5 mb-2 font-medium text-sm text-[var(--dash-text-secondary)]">Currency</div>
                                    <div className="flex gap-2">
                                        {['USD', 'INR'].map((c) => (
                                            <button
                                                ...
                                                className={`px-3 py-1.5 text-sm rounded-lg border transition-all ${currency === c ? 'bg-[var(--dash-text-primary)] text-white border-transparent' : 'bg-white border-[var(--dash-border)] text-[var(--dash-text-secondary)] hover:border-[var(--dash-text-muted)]'
                                                    }`}
                                            >
                                                {c}
                                            </button>
