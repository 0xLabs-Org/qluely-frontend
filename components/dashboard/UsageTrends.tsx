...
export function UsageTrends({ data }: UsageTrendsProps) {
    ...
    const insight = useMemo(() => {
        ...
        const currentWeek = chartData.slice(-7).reduce((a, b) => a + b, 0);
        const prevWeek = chartData.slice(-14, -7).reduce((a, b) => a + b, 0);
        if (prevWeek === 0) return null;
        const change = Math.round(((currentWeek - prevWeek) / prevWeek) * 100);
        if (change > 0) return `Your meeting usage increased ${change}% this week.`;
        if (change < 0) return `Your meeting usage decreased ${Math.abs(change)}% this week.`;
        return 'Your meeting usage is consistent this week.';
    }, [chartData]);
        ...
        <div className="dash-card">
                ...
                <div className="flex gap-0.5 p-[3px] rounded-lg bg-[var(--dash-elevated)] border border-[var(--dash-border)]">
                    {(['Daily', 'Weekly', 'Monthly'] as Period[]).map((period) => (
                        <button
                            ...
                            className={`px-3.5 py-[6px] text-[12px] font-medium rounded-md transition-all duration-200 ${selectedPeriod === period
            ...
            {insight && (
                <div className="flex items-center gap-2 mb-6 mt-2">
                    ...
                    <p className="text-[13px] text-[var(--dash-text-secondary)] font-medium">
                        {insight}
                    </p>
                </div>
            )}
            ...
            <div className="relative h-56 mt-4">
                    ...
                    {chartData.map((value, index) => {
                        ...
                        return (
                            <div key={index} className="flex-1 flex flex-col items-center justify-end h-full group/bar relative">
                                ...
                                <div
                                    className={`w-full max-w-[32px] rounded-t-[4px] transition-all duration-500 ease-out cursor-pointer ${isHighlighted
                ...
                <div className="absolute left-8 right-0 bottom-0 flex">
                    {labels.map((label, index) => (
                        <span
                            key={index}
                            className={`flex-1 text-center text-[10px] text-[var(--dash-text-muted)] font-mono tabular-nums ${index % showLabelEvery !== 0 && index !== labels.length - 1 ? 'invisible' : ''
                                }`}
                        >
